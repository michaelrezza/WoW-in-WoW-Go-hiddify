...
def get_best_ipv6():
    ...
    try:
        ...
        print(f"Fetched IPv6: {ipv6}")  # Debug output
        if ":" in ipv6:
            return ipv6
        else:
            print("⚠️ Invalid IPv6 fetched. Falling back to default.")
            return None
    except Exception as e:
...
def main():
    ...
    if not ipv6:
        # Fall back to a manually set default IPv6 for testing
        ipv6 = "2001:db8::1"  # Example IPv6 address, replace with actual fallback
        print(f"Using fallback IPv6: {ipv6}")
