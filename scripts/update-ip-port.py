...
def get_best_ipv6():
    ...
    try:
        ...
        print(f"❌ Error: warp-go.sh script not found. Ensure it is in ./scripts/ directory.")
...
def update_wireguard_config(config, ipv6, port):
    ...
    wireguard_template = {
        "type": "wireguard",
        "tag": "IP-> Your IP address", # Keep the tag from the original template or keep it constant
        "local_address": ["172.16.0.2/32", ipv6],
        "private_key": generate_private_key(),
        "server": ipv6, # Use IPv6 as server
        "server_port": port,
    ...
    for outbound in config.get("outbounds", []):
        if outbound.get("type") == "wireguard" and outbound.get("tag") == "IP-> Your IP address": # Check the tag as well
    ...
    if not updated:
        ...
        config["outbounds"].append(wireguard_template) # Add to outbounds if not found

    # Update the 'IP-> Main' section (if it exists and needs IP change)
    for outbound in config.get("outbounds", []):
        if outbound.get("type") == "wireguard" and outbound.get("tag") == "IP-> Main":
            old_ipv4_server = outbound.get("server", "[old_ipv4]") # To track changes
            outbound["server"] = ipv6 # Update only the server IP, keep port and key constant
