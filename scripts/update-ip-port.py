...
# Ensure 'outbounds' key exists and is a list
if 'outbounds' not in config or not isinstance(config['outbounds'], list):
    print("⚠️ 'outbounds' key not found or invalid. Resetting...")
    config['outbounds'] = []

# If outbounds is empty, add a default WireGuard configuration
if not config['outbounds']:
    print("⚠️ No WireGuard configuration found. Adding a default one...")
    config['outbounds'].append({
        "type": "wireguard",
        "local_address": ["192.168.1.1", best_ipv6],
